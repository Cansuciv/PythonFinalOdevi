...
# Veritabanı bağlantısı oluşturma veya var olan veritabanına bağlanma
vt = sqlite3.connect("kullanici_bilgileri.db")
imlec = vt.cursor()

# Kullanıcı tablosunu oluşturma (eğer daha önce oluşturulmadıysa)
sorgu = """CREATE TABLE IF NOT EXISTS kullanici (
...
while True:
    ...
    giris = input("\n\nSisteme admin olarak mı giriş yapmak istiyorsunuz kullanıcı olarak mı giriş yapmak istiyorsunuz (admin/kullanıcı): ")
    ...
    if giris.lower()=="kullanıcı":
        ...
        if secim==1:
            ...
            imlec.execute("""INSERT INTO kullanici (ad_soyad, kullanici_adi, tel_no, e_posta, tc_no, adres, sifre)
                      VALUES (?, ?, ?, ?, ?, ?, ?)""", (ad_soyad, kullanici_adi, tel_no, e_posta, tc_no, adres, sifre))
...
ile değiştirmek için 4'e, e-posta dresi ile değiştirmek için 5'ye basınız:""")
    ...
    #admin ekranı.............................................................
        ...
        df=pd.read_csv("kullanici-bilgileri.csv")
        print(df)
        ...
        while True:
            ...
            Cikis=input("\nSistemden çıkmak için '1'e basınız. Devam etmek için herhangi bir yere basınız\n")
            if Cikis=="1":
                break
            else:
                continue
